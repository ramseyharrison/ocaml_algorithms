...
let unsorted_list mtd () =
  Alcotest.(check (list int)) "same lists" (mtd [ 10; 5; 1; 8 ]) [ 1; 5; 8; 10 ]

let empty_list mtd () = Alcotest.(check (list int)) "same lists" (mtd []) []

let singleton_list mtd () =
  Alcotest.(check (list int)) "same lists" (mtd [ 1 ]) [ 1 ]
    ...
    [
      ( "Unsorted List",
        [
          ...
          Alcotest.test_case "Insertion" `Quick
            (unsorted_list Insertion_sort.sort);
      ...
      ( "Singleton List",
        [
          ...
          Alcotest.test_case "Insertion" `Quick
            (singleton_list Insertion_sort.sort);
